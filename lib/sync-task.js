import mu, { sparqlEscapeDateTime } from 'mu';
import { querySudo as query, updateSudo as update } from '@lblod/mu-auth-sudo';
import fetch from 'node-fetch';

import { PUBLIC_GRAPH, SERVICE_NAME, START_FROM_DELTA_TIMESTAMP, SYNC_FILES_ENDPOINT } from '../config';
import DeltaFile from './delta-file';

// TODO make configurable
const TASK_NOT_STARTED_STATUS = `http://lblod.data.gift/${SERVICE_NAME}-sync-task-statuses/not-started`;
const TASK_ONGOING_STATUS = `http://lblod.data.gift/${SERVICE_NAME}-sync-task-statuses/ongoing`;
const TASK_SUCCESS_STATUS = `http://lblod.data.gift/${SERVICE_NAME}-sync-task-statuses/success`;
const TASK_FAILED_STATUS = `http://lblod.data.gift/${SERVICE_NAME}-sync-task-statuses/failure`;
...
async function scheduleSyncTask() {
  ...
  if (result.results.bindings.length) {
    ...
    const uuid = mu.uuid();
    // TODO make configurable
    const uri = `http://lblod.data.gift/gelinkt-notuleren-functionarissen-consumer-sync-tasks/${uuid}`;
    await update(`
      PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
      PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
      PREFIX dct: <http://purl.org/dc/terms/>
      ...
      INSERT DATA {
        GRAPH <${PUBLIC_GRAPH}> {
          <${uri}> a ext:SyncTask ;
             mu:uuid "${uuid}" ;
             adms:status <${TASK_NOT_STARTED_STATUS}> ;
             dct:creator <http://lblod.data.gift/services/gelinkt-notuleren-functionarissen-consumer> ;
  ...
  if (result.results.bindings.length) {
    ...
    if (!latestDeltaTimestamp) {
      ...
      if (START_FROM_DELTA_TIMESTAMP) {
        ...
        console.log(
            `No configuration as of when delta's should be consumed. Starting consuming from sync task creation time ${b['created'].value}.`);
